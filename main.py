...
def main(fn,
    ...
    df, filtered_right, filtered_left, \
        intensity_right, intensity_left = initialPreprocessFile(file_name= fn, 
                                                                dir_saved_files= dir_saved_files)
    

    arr_illum_start_end = getIlluminationChangeValsTimestamps(df= df, 
    ...
    RAPD_score_arr, all_filtered_timestamp_to_df, all_filtered = getRAPDGraphPoints(hm_illum=hm_illum, 
                                        reps=reps, 
    ...
    try:
        if show_diameter_plot:
            saved_img_fn = getDiameterandRAPDGraph(RAPD_score_arr= RAPD_score_arr, 
                                    file_name= fn,
                                    df= df,
                                    filtered_right= filtered_right,
                                    filtered_left= filtered_left,
            ...
            saved_img_fn = getRAPDGraph(RAPD_score_arr= RAPD_score_arr, 
                    file_name= fn,
                    saved_image_dir= saved_image_dir)
...
if __name__ == "__main__":
    ...
    args = sys.argv
    if len(args) < 2:
        dir_csv_input = input("Enter current working directory and csv file name (with a space in between): ")
